...
if not rank:
        madx_command = str('/afs/cern.ch/eng/sl/MAD-X/pro/releases/5.02.00/madx-linux64 < ' + p['MADX_File'])
...
if sts['turn'] < 0:
	...
	for i in p:
        ...
        # Used for manually setting the initial distribution twiss
        # i.e. introducing a betatronic or dispersion mismatch
	# ~ twiss_dict = dict()	
...
#-----------------------------------------------------------------------
		...
		Particle_distribution_file = generate_initial_distribution_from_BLonD(p, Lattice, output_file='input/ParticleDistribution.in', summary_file='input/ParticleDistribution_summary.txt')
	...
	if turn in sts['turns_print']:
		...
		if not rank:
			with open(status_file, 'w') as fid:
                ...
                if(s['Indirect_Space_Charge'] and s['Print_SC_Grid']):
                        if not rank:
